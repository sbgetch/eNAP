...
function fBuildDocumentFavorite() {
  ...
  const oDocumentFavoriteHolder = $("<div/>", {
    id: "menu-holder-favorite",
  })
    .appendTo("body")
    .addClass("menu-holder")
    .css("left", () => $("#menu-button-favorite").offset().left);

  const cFavoriteNode = $($oDocRoot).find("favorite");
  cFavoriteNode.each((cFavoriteNode_i, cFavoriteNode_o) => {
    const oLinkContainter = $("<div/>").appendTo(oDocumentFavoriteHolder);
...
function fBuildDocumentFooter() {
  jqXHR.done(() => {
    const sDocYear = $oDocRoot.getAttribute("released");
    const sThisYear = new Date().getFullYear();
    const sDate =
      sDocYear == sThisYear ? sThisYear : `${sDocYear}-${sThisYear}`;

    $("<div/>", {
      id: "copyright",
      html:
        `${
          "<span>Vertiv Confidential and Proprietary Information</span>" +
          "<br/>&copy "
        }${sDate} Vertiv Co. All Rights Reserved.` +
...
function fBuildDocumentHistory() {
  if ($("div[id|=revision-history]")[0]) {
    jqXHR.done(() => {
      ...
      const cRevisedYear = [];
      const cRevisedNode = $($oDocRoot).find("log");
      $(cRevisedNode).each((cRevisedNode_i, cRevisedNode_o) => {
        const oDate = new Date($(cRevisedNode_o).attr("date"));
        !isNaN(oDate) && $.inArray(oDate.getFullYear(), cRevisedYear) == -1
          ? cRevisedYear.push(oDate.getFullYear())
          : null;
      });
      ...
      if (cRevisedYear.length > 1) {
        ...
        const oFilterDiv = $("<div/>", {
          id: "revision-filter",
        }).appendTo($("#revision-history"));
        const oFilterTxt = $("<span/>", {
        ...
        $(cRevisedYear).each((cRevisedYear_i, cRevisedYear_v) => {
          $("<option/>", {
...
function fBuildDocumentHistory_prep(cHistoryNode) {
  const oHistoryDiv = $("div[id|=revision-history]");
  const sHistoryCol = "Revision, Topic, Revision Details";
  const sHistoryTyp = "date, link, text";
  const oHistoryTab = fGenericBuildTable(oHistoryDiv, sHistoryCol, sHistoryTyp);
  const oLoadMessage = fStyleLoadingIndicator(oHistoryDiv);
...
function fBuildDocumentHistory_build(
  oHistoryTable,
  cHistoryNode,
  oLoadMessage
) {
  if (cHistoryNode.length > 0) {
    const cHistorySplice = cHistoryNode.splice(0, 5);

    $(cHistorySplice).each((cHistorySplice_i, cHistorySplice_o) => {
      const sDate = $(cHistorySplice_o).attr("date");
      const sTopic = fGenericShortenText(
        $(cHistorySplice_o).closest("topic").children("title").text(),
        30
      );
      const sLink = fGenericBuildHyperLink(
        sTopic,
        $(cHistorySplice_o).closest("topic").children("link").text()
      );
      let sDesc = fGenericShortenText($(cHistorySplice_o).text(), 100);
      ...
      sLink.css({
        background() {
          ...
          if (
            ($(cHistorySplice_o).parent().parent().attr("wip") &&
...
function fBuildDocumentMenu() {
  ...
  const cHistory = fTrackDocumentHistory();
  jqXHR.done(() => {
    // fBuildPartsButton();
...
function fSetDocumentMenuWidth(menu) {
  const menuSpan = menu.children("span");
  let menuWidth = 0;
...
function fBuildDocumentSafety() {
  ...
  const oDocumentSafetyHolder = $("<div/>", {
    id: "menu-holder-safety",
  })
    .appendTo("body")
    .addClass("menu-holder")
    .css("left", () => $("#menu-button-safety").offset().left);

  const sFileName = $("meta[name=Filename]").attr("content");
  let cSafetyNode = $($oDocRoot).find("safety");
  cSafetyNode = cSafetyNode
    .find(`applicability:contains(${sFileName})`)
    .parent();

  cSafetyNode.each((cSafetyNode_i, cSafetyNode_o) => {
    const oLink = $("<a/>", {
...
function fBuildRootMap() {
  const oDocumentMapHolder = $("<div/>", {
    id: "menu-holder-map",
  })
    .appendTo("body")
    .addClass("menu-holder");
  const oSectionHolder = $("<div/>")
    .appendTo(oDocumentMapHolder)
    .addClass("menu-holder-section");
  const cFileNode = $($oDocRoot).children("topic");

  cFileNode.each((cFileNode_i, cFileNode_o) => {
    const oLinkContainter = $("<div/>")
...
function fBuildTopicHert() {
  ...
  const oDocumentHertHolder = $("<div/>", {
    id: "menu-holder-hert",
  })
    .appendTo("body")
    .addClass("menu-holder")
    .css("left", () => $("#menu-button-hert").offset().left);

  const sFileName = $("meta[name=Filename]").attr("content");
  let cHertNode = $($oDocRoot).find("hert");
  cHertNode = cHertNode.find(`applicability:contains(${sFileName})`).parent();

  cHertNode.each((cHertNode_i, cHertNode_o) => {
    const oLink = $("<a/>", {
...
function fBuildTopicHistButton() {
  ...
  let iHistLogs = 0;
  cFileNode.each((cFileNode_i, cFileNode_o) => {
    iHistLogs += $(cFileNode_o).children("revision").find("log").length;
  });
  if (iHistLogs > 0) {
    $("<span/>", {
...
function fBuildTopicHistory() {
  ...
  const oDocumentHistoryHolder = $("<div/>", {
    id: "menu-holder-history",
  })
    .appendTo("body")
    .addClass("menu-holder")
    .css("left", () => $("#menu-button-history").offset().left);
  ...
  cFileNode.each((cFileNode_i, cFileNode_o) => {
    const cRevisionNode = $(cFileNode_o).children("revision").find("log");
    cRevisionNode.each((cRevisionNode_i, cRevisionNode_o) => {
      $("<div/>", {
        html: `<span>${$(cRevisionNode_o).attr("date")}</span>   ${$.trim(
          $(cRevisionNode_o).text()
        )
          .replace(/\[/g, "<")
          .replace(/\]/g, ">")}`,
      })
        ...
        .css({
          background() {
            ...
            if (
              $(cRevisionNode_o).closest("topic[wip]").length > 0 &&
              $(cRevisionNode_o)
                .closest("topic[wip]")
                .attr("wip")
                .toLowerCase()
...
function fBuildTopicIdButton() {
  //   const sFileName = $('meta[name=Filename]')
  //     .attr('content')
  //     .toUpperCase()
  //     .replace('.HTM', '');
...
function fBuildTopicMapButton(cHistory) {
  ...
  const bIsRootNode = !!$($oDocRoot)
    .find(`link:contains(${sFileName})`)
    .parent("doc")[0];
  const bIsTopicNode = !!cFileNode[0];
  const bHasParentNode = !!cFileNode.siblings("link")[0];
  const bIsCollection = !!cFileNode.parents().filter(function () {
    return $(this).prop("tagName").toLowerCase().indexOf("collection") > -1;
  })[0];
  ...
  $("h1").html(
    $("<span />", {
      ...
      background() {
        return !bProduction &&
          (cFileNode.attr("wip") || cFileNode.closest("topic[wip*=new]")[0])
          ? "yellow"
          : $("this").css("background");
      },
    })
              ...
              if (bHasParentNode) {
                ...
                for (let i = cHistory.length - 1; i >= 0; i--) {
                  if ($.inArray(cHistory[i], cSubTopic) == -1) {
                    ...
                    const oHistNode = $($oDocRoot)
                      .find(`link:contains(${cHistory[i]})`)
                      .parent("topic");
                    const bHistIsCollection = !!oHistNode
                      .parents()
                      .filter(function () {
                        return (
                          $(this)
                ...
                location.href =
                  sTargetIndex || $($oDocRoot).children("link").text();
              }
            } else {
              fBuildTopicMap();
            }
...
function fBuildTopicMap() {
  ...
  // trace ancestors
  cFileNode.each((cFileNode_i, cFileNode_o) => {
    const cParentNode = $($(cFileNode_o).parents().toArray().reverse());
    const oSectionHolder = $("<div/>")
      .appendTo(oDocumentMapHolder)
      .addClass("menu-holder-section");

    cParentNode.each((cParentNode_i, cParentNode_o) => {
      const oLinkContainter = $("<div/>")
        .appendTo(oSectionHolder)
        .append(
          $("<a/>", {
    ...
    // get siblings
    const cSiblingNode = $(cFileNode_o).parent().children("topic");
    cSiblingNode.each((cSiblingNode_i, cSiblingNode_o) => {
      if ($(cSiblingNode_o).children("link").text() == sFileName) {
        const oSpanContainter = $("<div/>")
          .appendTo(oSectionHolder)
          .append(
            $("<span/>", {
        ...
        const oLinkContainter = $("<div/>")
          ...
          .css({
            display() {
              if (
                ($(cSiblingNode_o).attr("wip") &&
                  $(cSiblingNode_o).attr("wip").toLowerCase().indexOf("new") >=
                    0) ||
                ($(cSiblingNode_o).closest("topic[wip]").length > 0 &&
                  $(cSiblingNode_o)
                    .closest("topic[wip]")
...
function fBuildTopicPreview() {
  if ($("#preview").length > 0) {
    jqXHR.done(() => {
      ...
      cFileNode.each((cFileNode_i, cFileNode_o) => {
        const cChildTopic = $(cFileNode_o).children("topic");
        cChildTopic.each((cChildTopic_i, cChildTopic_o) => {
          const oLinkContainter = $("<div/>")
            .appendTo($("#preview"))
            .css({
...
function fCreateModalBackground(iActiveButton) {
  const oModalBgr = $("<div />", { id: "modal-bground" })
    .appendTo($("body"))
    .css("opacity", 0.6);
...
function fLoadRemindersToTechDoc() {
  if (!bProduction) {
    ...
    if ($("#hert-menu").length > 0) {
      sMessage =
        `${sMessage}- Delete the hard-coded HERT charts on this page.\t\r\n   ` +
        "HERT charts should be coded in CLR instead.r\n";
    }
    if ($("#topic-history").length > 0) {
      sMessage =
        `${sMessage}- Delete the hard-coded topic history on this page.\t\r\n   ` +
        "Topic history should be coded in CLR instead.r\n";
    }
    if ($("#tab-dynamic").length > 0) {
      sMessage =
        `${sMessage}- This page has an element with ID name 'tab-dynamic'.\t\r\n   ` +
        "Please change the ID to class instead.r\n";
    }
    if ($("#tab-selector").length > 0) {
      sMessage =
        `${sMessage}- This page has an element with ID name 'tab-selector'.\t\r\n   ` +
        "Please change the ID to class instead.r\n";
    }
    if ($("#def-popup").length > 0) {
      sMessage =
        `${sMessage}- This page has an element with ID name 'def-popup'.\t\r\n   ` +
        "Please change the ID to class instead.r\n";
    }
    if (
      $(".def-popup").length > 0 &&
      ...
      sMessage =
        `${sMessage}- The DL under the 'def-popup' element has no ID.\t\r\n   ` +
...
function fSetGenericDocumentClick() {
  ...
  $("body").click((event) => {
    let cTargetAncestor = $.merge(
      $(event.target),
      $(event.target).parentsUntil("body")
    );
    cTargetAncestor = cTargetAncestor
      .map(function () {
        return $(this).attr("class");
      })
      .get();
    if (
      $.inArray("menu-button", cTargetAncestor) > -1 ||
...
function fSetReviewSchematicLink() {
  if (!bProduction) {
    if ($("a[href*=sxh  ]")[0]) {
      const sServerURL = "http://amerptkorv01.int.vertivco.com";
      $("a[href*=sxh  ]").each((cSchemLink_i, cSchemLink_o) => {
        if (location.toString().match(/^http([s]?):\/\/.*/)) {
          $(cSchemLink_o).attr("target", "_blank");
          $(cSchemLink_o).attr("href", (i, sSchemLink) => {
          ...
          $(cSchemLink_o).click(() => {
            sSchemReminder =
              "You can only view schematics on the Review Server.\r" +
              "Would you like to open the TKO Review Server now?";
            bGoToRevServer = confirm(sSchemReminder)
              ? window.open(`${sServerURL}/tko`)
              : null;
            return false;
          });
        }
...
function fStyleDocumentTips() {
  $(".warning, .caution, .note").each((cTip_i, cTip_o) => {
    const sTipLabel = $(cTip_o)
      .html()
      .substring(0, $(cTip_o).html().indexOf(":") + 1);
    const sTipText = $(cTip_o).html().replace(sTipLabel, "");
    $(cTip_o).html("");
    const oTipLabel = $("<div>", {
      html: sTipLabel,
    })
      .appendTo(cTip_o)
      .addClass(() => `${$(cTip_o).attr("class")}-label`);
    const oTipHolder = $("<div>")
      .appendTo(cTip_o)
      .addClass(() => `${$(cTip_o).attr("class")}-text`);

    const oTipText = $("<span>", {
      html: sTipText,
    }).appendTo(oTipHolder);

    const iWidthLimit = $("body").width() * 0.85;
...
function fStyleFilterableTable() {
  ...
  cTable.each((cTable_i, cTable_o) => {
    ...
    const oFilterIcon = $("<img/>", {
      src: `${baseUrl}custom/css/graphic/ico_filter.gif`,
      title: "Filter",
      hspace: 5,
    }).appendTo(oFilterContainer);
  });
      ...
      },
      {
        ...
        complete() {
          oModalCanvass
            .css(
              "width",
              `${parseInt((oModalContent.width() / $("body").width()) * 100)}%`
            )
            .css("height", "auto")
            .append(
              $(oModalContent)
...
function fStyleTableCodeSelector() {
  ...
  const cTableDefinition = $("#tab-definition, .tab-definition");
  if (cTableDefinition.length > 0) {
    cTableDefinition.each((cTableDefinition_i, cTableDefinition_o) => {
      const cSegment = $(cTableDefinition_o).find("table");
      $(cSegment).parent().hide();
      cSegment.each((cSegment_i, cSegment_o) => {
        const sCode = $(cSegment_o).find("td")[0];
        $("<input/>", {
          type: "button",
          value: $(sCode).text(),
          mouseup() {
          ...
          const oOption = $("<div/>", {
            ...
            click() {
              $(cTableSet_o)
                ...
                .not($(this))
                .attr("class", "selector-table-option");
              $(this).toggleClass(
                "selector-table-option, selector-table-option-selected"
              );
...
function fAddMyCEtvLink() {
  const sFileName = $("meta[name=Filename]").attr("content").toLowerCase();
  // var sFileName = 'z2521100.htm'.toLowerCase();

  jqXHR.done(() => {
    const oTechTipNode = $($oDocRoot)
      .find(`applicability:contains('${sFileName}')`)
      .parent("mycetv");
    if (oTechTipNode[0]) {
      const divBox = $("<div />").addClass("quick-box").insertAfter("h1");
      const divLabel = $("<div />", {
        ...
        .addClass("label")
        .appendTo(divBox);
      const divIndex = $("<div />").addClass("icon-index").appendTo(divBox);
      oTechTipNode.each(function () {
        const sDesc = $(this).find("title").text();
        const sLink = $(this).find("link").text();

        const oDiv = $("<div />").appendTo(divIndex);
        const oLink = $("<a />", {
          text: sDesc,
...
function fAddMyCEtvLink2() {
  jqXHR.done(() => {
    const oHold = $("div[class^=mycetv]").hide();
    if (oHold[0]) {
      oHold.each(function () {
        const sClass = $(this).text().replace(" ", "").split(",");
        const divBox = $("<div />").addClass("quick-box").insertAfter($(this));
        const divLabel = $("<div />", {
        ...
        for (i = 0; i < sClass.length; i++) {
          const oTechTipNode = $($oDocRoot)
            ...
            .parent("mycetv");
          const sDesc = $(oTechTipNode).find("title").text();
          const sLink = $(oTechTipNode).find("link").text();
...
function fSetSchemLink() {
  ...
  function fCreateSxhLink(xml) {
    const sxh = $("span[class^=sxh]");
    $(sxh).each(function () {
      const ref = $(this).attr("class").split("_")[1];
      const x_mdb = $(xml)
        .find("ref")
        .filter(function () {
          if ($(this).text() == ref) {
        ...
        .siblings("mdb");
      const rev = x_mdb.siblings("rev").text();
      const dwg = x_mdb.siblings("drawing").text();
