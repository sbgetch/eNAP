...
function fStyleModalNotification() {
  ...
  const modalContentEl = $("div[id|=modal-notification]");
  if (!modalContentEl.length) return; // Exit if modal not present

  const content = modalContentEl.text().trim();
  const sModalKey = `modal${hashModalKey(content)}`;

  // Load all modal states from localStorage (or empty object if none)
  let modalStates = JSON.parse(localStorage.getItem("modalStates")) || {};
  ...
  if (lastShown !== todayKey) {
    ...
    // Save/update this modal's state
...
function fStyleModalLoader(oModalContent, oModalCanvass) {
  ...
  oModalCanvass.hide().find(".modal-button").hide();
...
//still need to verify if needed a revamp
    ...
    $(cStoredForm_o)
      ...
      .val(function () {
        if (localStorage.getItem($(this).attr("name"))) {
          ...
          fStyleConditionalEl($(this), localStorage.getItem($(this).attr("name")));
          return localStorage.getItem($(this).attr("name"));
        }
        return $(this).val();
      })
