...
function z25_fLoadSamLibCalc() {
  ...
  function handleGenericChange() {
    ...
    if (sRating && sPnu1605 && sCellNum && sStringNum) {
      const oSetting = exls1SamsungBatterySettings.find(
  ...
  function handleRatingChange() {
    ...
    const arrStr = [];
    exls1SamsungBatterySettings.filter((st) => st.rating === selectedRating).forEach((st) => fUniqueArray(st.stringnum, arrStr));
...
function z25_fLoadHplLibCalc() {
  ...
  function handleGenericChange() {
    ...
    if (sRating && sPnu1605 && sCelTemp && sStringNum) {
      const oSetting = exls1HplBatterySettings.find(
  ...
  function handleRatingChange() {
    ...
    const arrStr = [];
    exls1HplBatterySettings.filter((st) => st.rating === selectedRating).forEach((st) => fUniqueArray(st.stringnum, arrStr));
...
function z23_fLoadSamLibCalc() {
  ...
  function handleChange() {
    ...
    if (sRating && sPnu605 && sCellNum && sStringNum) {
      const oSetting = nx255SamsungBatterySettings.find(
...
function z23_fLoadHPLLibCalc() {
  ...
  function handleChange() {
    ...
    if (sRating && sPnu605 && sCelTemp && sStringNum) {
      const oSetting = nx255HplBatterySettings.find(
...
function g26_fLoadPartsTab() {
  ...
  if (!loader) {
    ...
    loader.src =
      "https://napsapps.egain.services/media-server/public/inline/TMPROD67434354/fafe0023-e05b-4678-aa50-a8cc9093989e/theme_graphic/c7c1ef40-b13a-4bdb-8267-f02e53a37a56/cue_loading.gif";
    loader.id = "loader";
...
function p45_setDivContent() {
  ...
  if (year === 2025) {
    if (month === 11) {
      // December of the outgoing year
      div.innerHTML = "684279522";
    }
    return;
  }
  ...
  if (year === 2026) {
    ...
    if (month >= 0 && month <= 11) {
      div.innerHTML = passwordsByMonth[month];
    }
    return;
  }
  ...
  // Future years
  if (year > 2027) {
    div.innerHTML = "<strong>Time to Update the Passwords</strong>";
  }
}
/* ********************************* p45 ********************************* */
...
function p61_fCreateASCOPartsSel() {
  ...
  if (!tabHolder || !selKva || !selVdc || !selVac) return;
